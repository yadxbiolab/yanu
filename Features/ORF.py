...
class ExtractORF:
    ...
    def __codons(self, frame):
        start_coord = frame
        while start_coord + 3 <= len(self.seq):
            yield (self.seq[start_coord:start_coord + 3], start_coord)
            start_coord += 3

    def __longest_orf_in_seq(self, frame_number, start_codon, stop_codon):
        codon_posi = self.__codons(frame_number)
    ...
    def longest_ORF(self, start=None, stop=None):
        if stop is None:
            stop = ['TAA', 'TAG', 'TGA']
        if start is None:
            start = ['ATG']
        for frame in range(3):
            self.__longest_orf_in_seq(frame, start, stop)
